...
**Automatic catalog match after analysis** — the prompt (`VisionAnalysisPrompt.kt`) tells the model to be assertive when an inscription it actually read names a known product outright (e.g. text literally reading "Arduino Nano") rather than hedging on things it can genuinely see, while still refusing to invent a manufacturer/reference it can't justify. It also always returns `searchQueries` — ready-to-use keyword combinations (what it read + the object type), one or more depending on how many plausible angles it sees. Right after analysis completes (in both flows above), GridMind automatically fires all of those queries at DigiKey/Mouser/Adafruit/eBay in parallel and, if anything comes back, shows the results in a pop-up picker (`CatalogMatchPicker`) — a grid of thumbnails, names and manufacturers to choose from, the same visual pattern as Jellyfin's or TheMovieDB's artwork picker. Picking one routes straight into the existing catalog-confirm step (`PrefilledItemConfirmStep`) with real product data — the catalog becomes the source of truth for the name/image/reference, not the AI's guess. If nothing matches, or the user dismisses the picker, they fall through to today's AI-prefilled form — which still shows the same `searchQueries` as clickable chips ("Pas sûr ? Rechercher dans les catalogues :") for a manual, single-query re-search as a second fallback.

Backend-side, this reuses the existing single-query search machinery rather than adding a new pipeline: `ProductSearchService.searchMany(queries)` runs each query through the existing per-query provider fan-out (`search(query)`, itself already parallel across all 4 providers) and merges everything with the same `ProductGrouper` dedup rules used everywhere else, exposed as `GET /api/catalog/search-many?queries=...&queries=...`. Self-invocation from `searchMany` bypasses `search`'s `@Cacheable` proxy, so this path isn't cached — accepted, since it only ever runs once per AI analysis, not as a repeated identical search.
